...
- [ ] AC-4: 마이그레이션(`alembic upgrade head`)이 소분류('미분류' 외)가 지정된 기존 예산을 **삭제**하고, 소분류 '미분류' 예산만 대분류로 백필해 보존한다 — 업그레이드 후 budgets 테이블에 소분류 유래 행이 없고 '미분류' 예산이 대분류 이름으로 남아 있음을 DB 조회로 확인. *(개정 2026-06-12: 원래 "합산 병합·총액 보존"이었으나, 사용자가 /implement 입력에서 "아직 운영 단계가 아니고 개발 테스트 단계라 소분류까지 지정된 예산이 있는 경우 그냥 삭제해도 돼"로 결정하여 변경)*
