...
- **패널 본문에는 `panelBodyScroll`(lib/utils.ts)을 붙인다.** SEED `BottomSheetBody`는 `height: auto`라 스스로 스크롤하지 않아, 목록이 길면 시트가 자라 화면을 덮고 위쪽이 잘린 채 스크롤도 안 된다. 반대로 `SidePanelBody`는 이미 스크롤 컨테이너라 제한을 두면 안 된다. 이 유틸이 md 경계로 둘을 갈라준다 (`max-h-[60dvh] overflow-y-auto md:max-h-none md:overflow-visible`)
