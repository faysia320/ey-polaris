...
- **UI 변경 시 모바일 점검 (필수)**: 변경이 frontend UI에 닿으면 375px 뷰포트 기준(CLAUDE.md "모바일 대응")을 점검합니다. 브라우저 도구를 쓸 수 있으면 375px로 직접 확인하고, 불가하면 정적으로라도 점검(고정 px 너비, 반응형 유틸리티 부재, 넓은 콘텐츠의 오버플로 미처리)한 뒤 동적 미확인 사실을 보고서에 명시하세요. research.md에 모바일 AC가 빠져 있다면 그 자체를 [Medium] 이슈로 보고합니다 (research 단계의 계약 누락).
