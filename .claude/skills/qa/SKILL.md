...
작업 폴더에 `qa-report.md`를 저장하세요. **이 파일이 당신에게 허용된 유일한 쓰기입니다** — 코드 수정은 발견 사항이 확실해도 금지(수정은 `/implement`의 역할). Bash는 검증 실행 전용이며, 파일을 생성·수정하는 명령(리다이렉트 `>`, `mv`, `rm` 등)에 사용하지 마세요. 테스트가 임시 파일을 만들면 검증 후 정리합니다.
