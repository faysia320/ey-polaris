...
- **표본화**: AC당 대표 케이스 1회 + 엣지 1회로 충분합니다. 같은 assertion을 반복 실행하지 마세요. AC 밖에서 발견한 이상 신호는 **재현 1회 + 보고서 기록까지만** 하고 본 검증으로 복귀합니다(사이드 조사 타임박스). 깊은 추적은 실제 의심 신호가 있는 경로에만.
...
### Severity 기준 (기계적으로 적용)

판정은 이슈의 Severity에 걸려 있으므로, 등급은 직관이 아니라 아래 표로 정합니다. **경계가 애매하면 항상 높은 쪽**으로 올립니다.

| Severity | 기준 |
| --- | --- |
| **High** | 데이터 손상·유실, 크래시, 보안 취약점, AC를 형식만 충족하고 실질을 우회, 회귀(기존 동작 파괴) |
| **Medium** | 정상 경로는 동작하나 명세된 엣지·잘못된 입력에서 오동작/미처리 예외, 에러 핸들링 누락, 잘못된 응답 형식, 실사용 규모에서 체감되는 성능 문제, **UI 변경 시 375px 뷰포트에서 가로 스크롤·겹침·잘림** |
| **Low** | 동작에 영향 없는 품질 문제 — 네이밍, 중복, 컨벤션, 주석 불일치, 사소한 비효율 |
